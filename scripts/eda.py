...
def eda(input_path, output_dir):
    ...
    line_chart = alt.Chart(proportions).mark_line(
        ...
        x=alt.X('quality:Q', title='Quality Score', scale=alt.Scale(domain=[2.5, 9.5])),
        y=alt.Y('proportion:Q', title='Proportion', axis=alt.Axis(format='.0%')),
        color=alt.Color('color:N', title='Wine Type', scale=alt.Scale(domain=['red', 'white'], range=['#1f77b4', '#ff7f0e'])),
        tooltip=[
    ...
    for col in numerical_cols:
        ...
        ).properties(title=col, width=250, height=80)
        charts.append(chart)
    ...
    ).properties(width=350, height=200, title='Distribution of Wine Quality Scores')
